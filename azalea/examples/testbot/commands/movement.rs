...
pub fn register(commands: &mut Dispatcher) {
    commands.register(
        literal("goto")
            .executes(|ctx: &Ctx| -> eyre::Result<i32> {
    ...
    commands.register(
        literal("follow").executes(|ctx: &Ctx| -> eyre::Result<i32> {
            let source = ctx.source.lock();
            println!("got follow");
            // look for the sender
            let Some(entity) = source.entity() else {
    ...
    commands.register(literal("down").executes(|ctx: &Ctx| -> eyre::Result<i32> {
    ...
    commands.register(literal("south").executes(|ctx: &Ctx| -> eyre::Result<i32> {
        let source = ctx.source.lock();
        source.bot.set_direction(0., 0.)?;
        source.reply("ok");
        Ok(1)
    }));
    commands.register(literal("east").executes(|ctx: &Ctx| -> eyre::Result<i32> {
        let source = ctx.source.lock();
        source.bot.set_direction(-90., 0.)?;
        source.reply("ok");
        Ok(1)
    }));
    commands.register(literal("west").executes(|ctx: &Ctx| -> eyre::Result<i32> {
        let source = ctx.source.lock();
        source.bot.set_direction(90., 0.)?;
        source.reply("ok");
        Ok(1)
    ...
    commands.register(literal("stop").executes(|ctx: &Ctx| -> eyre::Result<i32> {
        let source = ctx.source.lock();
    ...
    commands.register(
        literal("forcestop").executes(|ctx: &Ctx| -> eyre::Result<i32> {
            let source = ctx.source.lock();
